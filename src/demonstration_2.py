...
def build_tree(preorder: List[int], inorder: List[int]) -> TreeNode:
    ...
    root, _ = recurse(preorder, preorder_index,
                      inorder_index_map, 0, len(inorder) - 1)
...
def recurse(preorder, preorder_index, inorder_index_map, inorder_start, inorder_end):
    ...
    # left recursive call
    root.left, preorder_index = recurse(
        preorder, preorder_index, inorder_index_map, inorder_start, root_index - 1)

    # right recursive call
    root.right, preorder_index = recurse(
        preorder, preorder_index, inorder_index_map, root_index + 1, inorder_end)
